...
namespace boost {
  ...
  public:
    ...
    filtered_graph(const Graph& g, EdgePredicate ep)
        : Base(g), m_edge_pred(ep),
          m_out_edge_pred(m_edge_pred, m_vertex_pred, *this),
          m_in_edge_pred(m_edge_pred, m_vertex_pred, *this),
          m_all_edge_pred(m_edge_pred, m_vertex_pred, *this) {}
    ...
    OutEdgePred m_out_edge_pred;
    InEdgePred m_in_edge_pred;
    EdgePred m_all_edge_pred;
  };
  ...
  edges(const filtered_graph<G, EP, VP>& g)
  {
    ...
    return std::make_pair(iter(g.m_all_edge_pred, f, l),
                          iter(g.m_all_edge_pred, l, l));
  }
  ...
  source(const typename filtered_graph_base<G>::edge_descriptor& e,
  ...
  inline
  typename filtered_graph_base<G>::vertex_descriptor
  target(const typename filtered_graph_base<G>::edge_descriptor& e,
         const filtered_graph_base<G>& g)
  {
            ...
            const filtered_graph<G, EP, VP>& g)
  {
    ...
    auto range = out_edges(u, g.m_g);
    return std::make_pair(iter(g.m_out_edge_pred, range.first, range.second),
                          iter(g.m_out_edge_pred, range.second, range.second));
  }
...
template <typename G, typename EP, typename VP>
           ...
           const filtered_graph<G, EP, VP>& g)
  {
    ...
    auto range = in_edges(u, g.m_g);
    return std::make_pair(iter(g.m_in_edge_pred, range.first, range.second),
                          iter(g.m_in_edge_pred, range.second, range.second));
  }
             ...
             const filtered_graph<G, EP, VP>& g)
  {
    ...
    return std::make_pair(iter(g.m_out_edge_pred, f, l),
                          iter(g.m_out_edge_pred, l, l));
  }
